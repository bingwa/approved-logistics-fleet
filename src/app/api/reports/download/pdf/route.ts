...
// IMPORTANT: Add these exports for production
...
async function getBrowser() {
  if (process.env.NODE_ENV === 'production' || process.env.VERCEL_ENV) {
    // Production/Vercel environment - use @sparticuz/chromium
    ...
    // Development environment - use system puppeteer
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      details: 'PDF generation failed - check server logs for details'
    }, { status: 500 })
